...
def update_centroids(clusters):
    ...
    for c, points in clusters.items():
        if points:
            new_centroid = [sum(p[dim] for p in points) / len(points) for dim in range(len(points[0]))]
...
def k_means(filename, k, max_iterations=100, tol=1e-4):
    points = []
...
centroids, clusters = k_means('Housing.csv', 2)
print_d(centroids, clusters)
